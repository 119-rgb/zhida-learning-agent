...
class MySqlConversationTest {
    @Test void restoresOrderedContextFromMySql() {
        var source = new DriverManagerDataSource(System.getenv("DB_URL"), System.getenv("DB_USERNAME"), System.getenv("DB_PASSWORD"));
        new org.springframework.jdbc.datasource.init.ResourceDatabasePopulator(
                new org.springframework.core.io.ClassPathResource("db/conversation-schema.sql")).execute(source);
        ...
        transaction.executeWithoutResult(status -> {
            ...
            assertThat(ConversationContext.restore(rebuilt.recentMessages("context-test", id), "我想学什么？"))
                    .extracting(org.springframework.ai.chat.messages.Message::getText)
                    .containsExactly("我的学习目标是 Java", "好的，我们学习 Java", "我想学什么？");
            var tasks = new TaskRepository(jdbc, transaction, rebuilt, new com.fasterxml.jackson.databind.ObjectMapper());
            String taskId = UUID.randomUUID().toString();
            tasks.begin("context-test", id, taskId, "继续");
            tasks.record("context-test", id, new com.zhida.agent.application.AgentEvent(1, taskId, "tool.completed",
                    java.time.OffsetDateTime.now(), java.util.Map.of("toolName", "current_date")));
