...
class ConversationRepositoryTest {
    ...
    private ConversationRepository open() {
        var source = new DriverManagerDataSource("jdbc:h2:file:" + directory.resolve("history").toString().replace('\\', '/') + ";MODE=MySQL", "sa", "");
        new ResourceDatabasePopulator(new ClassPathResource("db/conversation-schema.sql")).execute(source);
        return new ConversationRepository(new JdbcTemplate(source), new TransactionTemplate(new DataSourceTransactionManager(source)));
    }

    @Test void persistsAcrossReopenedConnectionsAndFiltersOwners() {
        var first = open();
        first.create("alice", "a", "RAG 是什么");
        first.append("alice", "a", "user", "RAG 是什么");
        first.append("alice", "a", "assistant", "先检索资料，再生成回答。");
        var reopened = open();
        assertThat(reopened.messages("alice", "a")).extracting(ConversationRepository.Message::role)
                .containsExactly("user", "assistant");
        assertThat(reopened.list("bob")).isEmpty();
        assertThatThrownBy(() -> reopened.messages("bob", "a")).isInstanceOf(ResponseStatusException.class);
        ...
        assertThatThrownBy(() -> reopened.recentMessages("bob", "a")).isInstanceOf(ResponseStatusException.class);
        ...
        assertThatThrownBy(() -> reopened.deleteMemory("bob", memory.id())).isInstanceOf(ResponseStatusException.class);
