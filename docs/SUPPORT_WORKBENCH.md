...
| 会话过期后无法恢复 | 助手流式请求绕过了统一的 401 处理：JWT 过期后 token 一直留在 `sessionStorage`，页面既不弹登录框也不清会话，用户每发一条只看到「助手请求失败（401）」 | `streamAssistant` 自己处理 401：清除失效 token、回到登录态并提示「登录状态已过期，请重新登录」；浏览器验收覆盖「页面加载 401」与「助手接口 401」两条分支 |
