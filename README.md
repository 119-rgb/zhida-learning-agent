...
DeepSeek 聊天密钥不会自动用于 Embedding（DeepSeek 目前也没有 `/embeddings` 接口）。未配置时仍可启动页面和聊天，文档索引/检索会明确提示配置错误，不会用关键词检索代替向量检索。向量化会将文档片段与查询发送给所配置的供应商；费用与数据处理规则以该供应商为准。

### 没有第三方向量服务时：内置离线模型

如果只有聊天模型的 Key，可以把 `EMBEDDING_MODEL` 设为 `local`，改用随应用打包的离线向量模型（`all-MiniLM-L6-v2`，384 维，ONNX）：

```powershell
$env:EMBEDDING_MODEL="local"
# 不需要 EMBEDDING_BASE_URL 与 EMBEDDING_API_KEY
```

- 模型文件（约 79MB）直接包含在依赖 jar 内，**不需要额外下载，也不联网**，文档内容不会离开本机。
- `GET /api/health` 会返回 `embeddingMode`：`local`（离线模型）、`remote`（外部服务）或 `none`（都未配置）。
- 两种模式共用同一个向量文件，但索引身份不同（`local:` 与 `remote:模型名+地址`）。因此**在两种模式之间切换会要求重建索引**，不会把两种向量混进同一个库；原始文档保留，重新上传或点「重试」即可。
- 离线模型适合演示与学习；追求检索质量时应评估更强的向量模型。
...
`ZHIDA_CHUNK_SIZE`、`ZHIDA_CHUNK_OVERLAP`、`ZHIDA_TOP_K` 可按部署调整（默认 800 / 120 / 5）。分块越大，单个向量承载的语义越杂，越难被具体问题命中。实测同一份 882 字的产品手册（离线模型、余弦门槛 0.30）：

| 分块 | 命中「付款但服务未开通」 | 命中「已付款但服务未开通」 | 「付款后服务没有开通」最高分 |
| --- | --- | --- | --- |
| 800（1 块） | 0 命中 | 0 命中 | 0.341 |
| 300 / 60（3 块） | 2 命中（0.458） | 2 命中（0.474） | 0.496 |

改动分块参数不会自动重建已有索引（索引身份只记录模型与地址），需要在文档列表重新上传或点「重试」。

**已知边界**：相似度是绝对门槛，与问题无关的查询也可能擦线命中（实测「忘记密码」在该手册上得 0.371，高于 0.30 门槛）。因此「有依据」只代表检索到了相似片段，不代表语义一定相关；回答仍须带出可核对出处，让用户自己判断。相对分数过滤是后续改进方向，本轮未实现。
